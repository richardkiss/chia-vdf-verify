...
use malachite_base::num::arithmetic::traits::{AddMulAssign, DivRem, NegAssign, SubMulAssign};
    ...
    while *r1 != 0i32 && &*r1 > l {
        ...
        if i == 0 {
            ...
            *r2 = std::mem::replace(r1, rem);
            co2.sub_mul_assign(&q, &*co1);
            std::mem::swap(co2, co1);
        } else {
            let ibb2 = Integer::from(bb2);
            let iaa1 = Integer::from(aa1);
            let ibb1 = Integer::from(bb1);
            let iaa2 = Integer::from(aa2);

            // r_new = r2*bb2 + r1*aa2, r1_new = r1*aa1 + r2*bb1
            let mut r = &*r2 * &ibb2;
            r.add_mul_assign(&*r1, &iaa2);
            let mut new_r1 = &*r1 * &iaa1;
            new_r1.add_mul_assign(&*r2, &ibb1);
            *r2 = r;
            *r1 = new_r1;

            let mut new_co2 = &*co2 * &ibb2;
            new_co2.add_mul_assign(&*co1, &iaa2);
            let mut new_co1 = &*co1 * &iaa1;
            new_co1.add_mul_assign(&*co2, &ibb1);
            *co2 = new_co2;
            *co1 = new_co1;
